...
|  请求体  | 类型  | 必要性 | 内容 |
| :----: | :----: | :----: | :----: |
| season | str | 必要 | 如：2024.10,2024.1,2024.4,2024.7（没有则不筛选季度）|
| tag | str | 必要 | 标签名，如"治愈"、"热血"等 |
| isfinish | int | 必要 | 1-已完结，0-未完结（没有则不筛选）|

**json回复:**

|  字段   | 类型  | 内容 | 备注 |
| :----: | :----: | :----: | :----: |
| data | array | 番剧数据 | 为空数组表示无匹配数据 |

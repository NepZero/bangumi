...
class Database
{
     ...
     */
    async get_id_according_tag(season = null,tag = null,isfinish = null){//默认值 可直接修改
        let connection;
        try {
            connection = await this.pool.getConnection();
            // 1. 先查tag_info表
            let tagSql = `SELECT DISTINCT id FROM tag_info WHERE tag = ?`;
            let [tagRows] = await connection.execute(tagSql, [tag]);
            if (tagRows.length === 0) {
                return [];
            }
            // 2. 整理id列表
            let idList = tagRows.map(row => row.id);
            // 3. 构造bangumi_info的查询
            let whereClauses = [];//构建where语句
            let params = [];//where语句中的参数列表 因为可能为空所以这么设计 理论来说页面的初加载必有默认值 可以设置 这样可以去掉if语句
            if (idList.length > 0) {
            ...
            if (isfinish !== null) {
                whereClauses.push(`isfinish = ?`);
                params.push(isfinish);
            }
            let sql = `SELECT * FROM bangumi_info`;
